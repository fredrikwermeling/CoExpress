# CoExpress - Gene Expression Correlation Analysis Tool

A web-based application for analyzing gene co-expression patterns from DepMap RNA-seq data. Part of the [Green Listed](https://greenlisted.cmm.se) tool family.

**Live App:** [https://fredrikwermeling.github.io/CoExpress/](https://fredrikwermeling.github.io/CoExpress/)
...
3. **Mutation Analysis Mode** - Compare expression between wild-type and mutant cells for a selected hotspot mutation (Welch's t-test)
...
- Color nodes by expression level (signed or absolute)
- Color nodes by uploaded statistics (LFC, FDR)
- Show expression values and SD in node labels
- Gene info tooltips on hover (via MyGene.info)
- Click nodes to hide them, with restore option
- Double-click nodes to open expression distribution
...
- Box plots of expression by cancer type (tissue)
- Box plots of expression by hotspot mutation status
...
- Welch's t-test comparing expression between WT and mutant cells
- Three mutation levels (0 = WT, 1 = one hotspot mutation, 2 = two or more)
- Filter by p-value threshold
- Expression distribution charts per mutation group
- **Compare by Cancer Type** - Δ Expr for top significant genes across each tissue, with pinned "All" row; clickable values open inspect with tissue filter applied
...
- Download expression charts as PNG/SVG/CSV
...
- **OmicsExpressionTPMLogp1HumanProteinCodingGenes** - RNA-seq expression (log2 TPM+1, 19,215 genes x 1,699 cell lines)
...
1. **Wait for data to load** - The app loads expression data on startup
...
5. **Explore Results**:
   ...
   - **Clusters tab**: Gene cluster assignments with mean expression and SD
   - **Mutation Analysis tab**: Differential expression results with compare tables (mutation mode)
...
├── README.md                       # This file
└── web_data/
    ├── expression.bin.gz           # Expression matrix (binary, gzip compressed)
...
- [Correlate](https://fredrikwermeling.github.io/correlation-web-app-/) - Same analysis engine for CRISPR gene effect data
