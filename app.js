...
class CorrelationExplorer {
    ...
    async loadData() {
        ...
        // Compute curated-collection memberships (mutation-based and
        // expression-based — expression IS loaded in CoExpress).
    ...
    async loadGeneEffects() {
        ...
        this.updateLoadingText('Decompressing expression data...');
    ...
    _showUpsetPlot() {
        ...
        let uhtml = `<div id="upsetDragHandle" style="display:flex; justify-content:space-between; align-items:center; padding:6px 10px; background:#f0fdf4; border-radius:8px 8px 0 0; cursor:move; user-select:none;">`;
        uhtml += `<span style="font-weight:600; font-size:12px;">UpSet \u2014 ${upsetLabel}</span>`;
        uhtml += `<span style="font-size:10px; color:#6b7280;">${cls.length} cell lines${filterCtx ? ' \u00b7 ' + filterCtx : ''}</span>`;
        uhtml += `<button onclick="document.getElementById('upsetPopup').remove()" style="background:none;border:none;font-size:16px;cursor:pointer;color:#999;">&times;</button>`;
        uhtml += `</div>`;
        uhtml += `<div style="padding:10px; overflow:auto; flex:1;">`;
        uhtml += `<div id="upsetPlotDiv" style="width:${plotW}px; height:${plotH}px;"></div>`;
        uhtml += `</div>`;
        uhtml += `<div style="display:flex; gap:6px; padding:6px 10px; border-top:1px solid #e5e7eb; align-items:center; flex-wrap:wrap;">`;
        uhtml += `<label style="font-size:10px;cursor:pointer;color:#374151;"><input type="checkbox" id="upsetShowCounts" ${this._upsetShowCounts ? 'checked' : ''} onchange="app._upsetToggle('counts')" style="margin:0 3px 0 0;vertical-align:middle;"> Counts</label>`;
        uhtml += `<label style="font-size:10px;cursor:pointer;color:#374151;"><input type="checkbox" id="upsetShowPct" ${this._upsetShowPct ? 'checked' : ''} onchange="app._upsetToggle('pct')" style="margin:0 3px 0 0;vertical-align:middle;"> %</label>`;
        uhtml += `<label style="font-size:10px;cursor:pointer;color:#374151;"><input type="checkbox" id="upsetShowNames" ${this._upsetShowNames ? 'checked' : ''} onchange="app._upsetToggle('names')" style="margin:0 3px 0 0;vertical-align:middle;"> Names</label>`;
        uhtml += `<span style="border-left:1px solid #d1d5db;height:14px;"></span>`;
        uhtml += `<button onclick="app._upsetExport('svg')" style="font-size:10px;padding:2px 8px;border:1px solid #d1d5db;border-radius:4px;cursor:pointer;background:#f9fafb;">SVG</button>`;
        uhtml += `<button onclick="app._upsetExport('png')" style="font-size:10px;padding:2px 8px;border:1px solid #d1d5db;border-radius:4px;cursor:pointer;background:#f9fafb;">PNG</button>`;
        uhtml += `</div>`;
        popup.innerHTML = uhtml;
        document.body.appendChild(popup);
    ...
    applyTissueBreakdownSelection(selectedTissues, selectedSubtypes = []) {
        ...
        if (selectedSubtypes && selectedSubtypes.length > 0) {
            const parent = selectedSubtypes[0].parent;
            const lineageSelect = document.getElementById('lineageFilter');
            lineageSelect.value = parent;
            lineageSelect.disabled = false;
            lineageSelect.style.opacity = '';
            this.excludedTissues = new Set();
            this.updateSubLineageFilter();
            // Set sublineage if single subtype selected
            if (selectedSubtypes.length === 1) {
                const subSelect = document.getElementById('subLineageFilter');
                if (subSelect) subSelect.value = selectedSubtypes[0].subtype;
            }
            this.updateHotspotCountsForCurrentFilters();
            return;
        }
    ...
    runMutationAnalysis() {
        ...
        setTimeout(() => {
            try {
                ...
                // Sort by Δ Expr (1+2 vs 0) ascending — most negative first
    ...
    displayMutationResults(resetSortIndicator = false) {
        ...
        const cols = [
            ...
            { col: 'mean_wt', label: `Mean Expr (${wtLabel})`, style: '' },
            { col: 'n_mut', label: `N (${mutLbl} 1+2)`, style: 'border-left: 2px solid #f97316;' },
            { col: 'mean_mut', label: `Mean Expr (${mutLbl} 1+2)`, style: '' },
            { col: 'diff_mut', label: 'Δ Expr (1+2v0)', style: 'border-left: 2px solid #d1d5db;' },
            { col: 'p_mut', label: 'p-value', style: '' },
            { col: 'n_2', label: `N (${mutLbl} 2${isT ? '+' : ''})`, style: 'border-left: 2px solid #dc2626;', cls: 'mut2-col' },
            { col: 'mean_2', label: `Mean Expr (${mutLbl} 2${isT ? '+' : ''})`, style: '', cls: 'mut2-col' },
            { col: 'diff_2', label: 'Δ Expr (2v0)', style: 'border-left: 2px solid #d1d5db;', cls: 'mut2-col' },
            { col: 'p_2', label: 'p-value (2v0)', style: '', cls: 'mut2-col' },
            { col: 'diff_2v1', label: 'Δ Expr (2v1)', style: 'border-left: 2px solid #d1d5db;', cls: 'mut2-col' },
        ...
        if (hasFusion) {
            const fusionCols = [
                ...
                { col: 'mean_fused', label: 'Mean Expr (F)', style: '' },
                { col: 'diff_fused', label: 'Δ Expr (F)', style: 'border-left: 2px solid #d1d5db;' },
    ...
    downloadMutationResults() {
        ...
        if (mr.additionalTransGene && mr.additionalTransLevel !== 'all') {
            csv += `# Fusion filter: ${mr.additionalTransGene} = ${mr.additionalTransLevel}\n`;
        }
        if (this._activeOncoprintFilters && this._activeOncoprintFilters.length > 0) {
            const parts = this._activeOncoprintFilters.map(f => `${f.gene} ${f.state === 'mut' ? 'Mut' : 'WT'}`);
            csv += `# Oncoprint filters: ${parts.join(', ')}\n`;
        }
        csv += `# Date: ${new Date().toISOString().slice(0, 10)}\n`;
        csv += '#\n';

        const hasFusion = mr.hasFusionData && mr.isTranslocation;
        let headers = ['Gene', 'N_WT', 'Mean_Expr_WT', 'N_1+2', 'Mean_Expr_1+2', 'Delta_Expr', 'pValue_1+2_vs_0',
                        'N_2', 'Mean_Expr_2', 'Delta_Expr_2vs0', 'pValue_2_vs_0',
                        'Delta_Expr_2vs1', 'pValue_2_vs_1'];
        if (hasFusion) {
            headers.push('N_Fused', 'Mean_Expr_Fused', 'Delta_Expr_Fused', 'pValue_Fused');
        }
    ...
    showGeneEffectDistribution(gene, tissueOverride, inspectHotspotOverride) {
        ...
        const makeHoverText = (d) => {
            let text = `${d.cellName}<br>${d.lineage}<br>Expr: ${d.ge.toFixed(2)}`;
        ...
        const titleText = `${gene} Expression by ${hotspotGene} ${statusLabel}`;
        ...
        document.getElementById('geneEffectTitle').textContent = `${gene} Expression by ${hotspotGene} ${isTranslocation ? 'Fusion' : 'Mutation'}`;
    ...
    downloadGeneEffectCSV() {
        ...
        const mr = this.mutationResults;
        let csv = `# Expression Distribution Data\n`;
            ...
            .forEach(c => {
                ...
                rowHtml += `
                    <td style="text-align: center; white-space: nowrap;">
                        <button class="btn btn-sm gene-effect-btn" style="padding: 2px 6px; font-size: 10px; background: #5a9f4a; color: white;" data-gene="${c.gene}">Expression</button>
    ...
    async downloadNetworkPNG() {
        ...
        if (colorByGeneEffect && this.results?.clusters) {
            ...
            if (colorGEType === 'signed') {
                ...
                ctx.fillText('Expression', legendX, gradY - 4);
...
`;
        ...
        if (colorByGeneEffect && this.results?.clusters) {
            ...
            if (colorGEType === 'signed') {
                ...
                svg += `  <text x="${legendX}" y="${gradY - 4}" class="legend-small">Expression (+/−)</text>\n`;
        ...
        if (colorByGeneEffect && this.results?.clusters) {
            ...
            if (colorGEType === 'signed') {
                ...
                if (colorLegend) colorLegend.innerHTML = `
                    <div class="legend-item">Expression</div>
                ...
                if (colorLegend) colorLegend.innerHTML = `
                    <div class="legend-item">Absolute Expression</div>
                    <div style="display: flex; align-items: center; gap: 4px;">
    ...
    toggleRemoveMode() {
        ...
        if (this.removeMode) {
            ...
            btn.style.color = '';
            if (helpText) helpText.textContent = 'Double-click node for Expression, edge for Correlation';
        }
    }
...
`;
    ...
    renderSinglePanelPlot(filteredData, gene1, gene2, hotspotGene, hotspotMode, searchTerms, fontSize, filterDesc = '', transOverlayGene = '', transOverlayMode = 'none', colorByCategory = '') {
        ...
        const xLabelText = `${gene1} Expression`;
        const yLabelText = `${gene2} Expression`;
    ...
    openGeneEffectModal(gene, view = 'tissue') {
        ...
        // Update UI
        document.getElementById('geneEffectTitle').textContent = `${geneUpper} - Expression Analysis`;
    ...
    // "Hotspot Mutation Inspect" button in the standalone expression modal:
    ...
    renderGeneEffectByTissue() {
        ...
        // When the user entered the GE modal via "Inspect Expression for
        ...
        const traces = stats.map((s, idx) => {
            ...
            return {
                ...
                hovertemplate: '<b>%{text}</b><br>Expression: %{x:.3f}<extra></extra>'
            };
        });
        ...
        const layout = {
            annotations: [
                ...
                { text: `${gene} Expression`, xref: 'paper', yref: 'paper', x: 0.5, y: -0.04, xanchor: 'center', yanchor: 'top', showarrow: false, font: { size: 17 }, _tsRole: 'xlabel' }
            ],
            xaxis: {
                zeroline: true,
    ...
    renderGeneEffectByHotspot() {
        ...
        const layout = {
            annotations: [
                { text: `<b>${gene} Expression by Hotspot Mutation</b>`, xref: 'paper', yref: 'paper', x: 0.5, y: 1.12, xanchor: 'center', yanchor: 'bottom', showarrow: false, font: { size: 15 }, _tsRole: 'title' },
                { text: `${gene} Expression`, xref: 'paper', yref: 'paper', x: 0.5, y: -0.04, xanchor: 'center', yanchor: 'top', showarrow: false, font: { size: 17 }, _tsRole: 'xlabel' }
            ],
            xaxis: {
                zeroline: true,
    ...
    renderGETable(stats, mode) {
        ...
        if (mode === 'tissue') {
            ...
            thead.innerHTML = `<tr>
                ...
                <th style="${headerStyle}" data-sort="mean" data-type="number">Mean Expr${sortIcon}</th>
            ...
            thead.innerHTML = `<tr>
                ...
                <th style="${headerStyle}" data-sort="mean0" data-type="number">Expr (${wtLbl})${sortIcon}</th>
                <th style="${headerStyle}; border-left: 2px solid #f97316;" data-sort="n1" data-type="number">N (${mutLbl} 1)${sortIcon}</th>
                <th style="${headerStyle}" data-sort="mean1" data-type="number">Expr (${mutLbl} 1)${sortIcon}</th>
                <th style="${headerStyle}; border-left: 2px solid #dc2626;" data-sort="n2" data-type="number">N (${mutLbl} 2${isT ? '+' : ''})${sortIcon}</th>
                <th style="${headerStyle}" data-sort="mean2" data-type="number">Expr (${mutLbl} 2${isT ? '+' : ''})${sortIcon}</th>
    ...
    showGEDetailedView(group, mode) {
        ...
        const formatStats = (stats, label) => {
            ...
            return `${label}: n=${stats.n}, Expr=${stats.mean.toFixed(2)}, SD=${stats.sd.toFixed(2)}`;
        };
        ...
        if (mode === 'hotspot') {
            ...
            const traces = [
                {
                    ...
                    hovertemplate: '<b>%{text}</b><br>%{customdata}<br>Expression: %{y:.3f}<extra>0 (WT)</extra>'
                }
            ];
            ...
            if (data1.length > 0) {
                traces.push({
                    ...
                    hovertemplate: '<b>%{text}</b><br>%{customdata}<br>Expression: %{y:.3f}<extra>1 mutation</extra>'
                });
            }
            ...
            if (data2.length > 0) {
                traces.push({
                    ...
                    hovertemplate: '<b>%{text}</b><br>%{customdata}<br>Expression: %{y:.3f}<extra>2 mutations</extra>'
                });
            }
            ...
            // Row 1: 0 (WT) stats
            statsAnnotations.push(`0 (WT): n=${stats0.n}, Expr=${stats0.mean.toFixed(2)}, SD=${stats0.sd.toFixed(2)}`);
            // Row 2: 1 mut stats
            if (stats1.n > 0) {
                statsAnnotations.push(`1 mut: n=${stats1.n}, Expr=${stats1.mean.toFixed(2)}${stats1.n > 1 ? `, SD=${stats1.sd.toFixed(2)}` : ''}`);
            }
            // Row 3: 2 mut stats
            if (stats2.n > 0) {
                statsAnnotations.push(`2 mut: n=${stats2.n}, Expr=${stats2.mean.toFixed(2)}${stats2.n > 1 ? `, SD=${stats2.sd.toFixed(2)}` : ''}`);
            }
            // Row 4: p-value
            if (pStr) {
            ...
            const layout = {
                title: { text: `${gene} expression by ${group} mutation status`, font: { size: 14 } },
                yaxis: { title: 'Expression', zeroline: true, zerolinecolor: '#374151' },
                showlegend: false,
                height: chartHeight,
                width: chartWidth,
        ...
        const traces = [
            {
                ...
                hovertemplate: '<b>%{text}</b><br>%{customdata}<br>Expression: %{y:.3f}<extra>All cells</extra>'
            },
            {
                type: 'box',
        ...
        const statsAnnotations = [];
        statsAnnotations.push(`All cells: n=${allStats.n}, Expr=${allStats.mean.toFixed(2)}, SD=${allStats.sd.toFixed(2)}`);
        statsAnnotations.push(`${group}: n=${groupStats.n}, Expr=${groupStats.mean.toFixed(2)}, SD=${groupStats.sd.toFixed(2)}`);
        if (!isNaN(pValue)) {
        ...
        const layout = {
            title: { text: `${gene} expression in ${group}`, font: { size: 14 } },
            yaxis: { title: 'Expression', zeroline: true, zerolinecolor: '#374151', zerolinewidth: 2 },
            showlegend: false,
            height: chartHeight,
            width: chartWidth,
    ...
    downloadGECellLineCSV() {
        ...
        // Get sublineage info
        let csv = 'Cell_Line_ID,Cell_Line_Name,Cancer_Type,Cancer_Subtype,Expression\n';
    ...
    showInlineCompareByTissue() {
        ...
        this._inlineCompareData = { title: `${gene} — Δ Expr by ${groupLabel} (${hotspotGene} WT vs ${mutLabel})`, headers: [groupLabel, 'N(WT)', 'Expr(WT)', `N(${mutLabel})`, `Expr(${mutLabel})`, 'Δ Expr'], refRows: allRow, sortableRows: otherRows, mode: 'tissue' };
    ...
    showInlineCompareByHotspot() {
        ...
        this._inlineCompareData = { title: `${gene} — Δ Expr by Additional ${typeLabel} (${mainHotspot} WT vs ${mutLabel})`, headers: [`${typeLabel} Filter`, 'N(WT)', 'Expr(WT)', `N(${mutLabel})`, `Expr(${mutLabel})`, 'Δ Expr'], refRows: refRow, sortableRows: otherRows, mode: 'hotspot' };
        this._renderInlineCompareTable();
    }
    ...
    showInlineCompareByTranslocation() {
        ...
        this._inlineCompareData = { title: `${gene} — Δ Expr by Fusion (${mainHotspot} WT vs ${mutLabel})`, headers: ['Fusion Gene', 'N(WT)', 'Expr(WT)', `N(${mutLabel})`, `Expr(${mutLabel})`, 'Δ Expr'], refRows: refRow, sortableRows: otherRows, mode: 'fusion' };
        this._renderInlineCompareTable();
    }
    ...
    renderCompareModal() {
        ...
        document.getElementById('mutCompareModalInfo').innerHTML =
            `<b>Δ Expr = Mean Expr(${mutLabel}) − Mean Expr(WT)</b> for ${d.hotspotGene} ${mutNoun}, stratified by ${modeLabel}. ` +
    ...
    async submitToEnrichr(genes) {
        ...
        formData.append('description', 'CoExpress');
    ...
    // Compute which cell lines belong to each curated collection. In
    // CoExpress this runs once after gene-effects (= expression) is loaded
    // so both mutation-based and expression-based subsets compute together.
    _computeCollectionMemberships() {
        ...
        // Expression-based collections. In CoExpress, geneEffects IS the
        // expression matrix and geneIndex is the expression gene index.
        if (this.geneEffects && this.geneIndex && this.geneIndex.size) {
            const nCL = this.nCellLines;
            const exprFor = (gene, clIdx) => {
                const gi = this.geneIndex.get(gene);
                if (gi === undefined) return NaN;
                return this.geneEffects[gi * nCL + clIdx];
            };
            ...
            const zFor = (gene) => {
                ...
                const off = gi * nCL;
                const vals = [];
                for (let i = 0; i < nCL; i++) { const v = this.geneEffects[off + i]; if (!isNaN(v)) vals.push(v); }
            ...
            if (upZ.length) {
                ...
                for (let i = 0; i < nCL; i++) {
                    let s = 0, n = 0;
                    for (const z of upZ) { const v = this.geneEffects[z.off + i]; if (!isNaN(v)) { s += (v - z.mean) / z.sd; n++; } }
                    for (const z of dnZ) { const v = this.geneEffects[z.off + i]; if (!isNaN(v)) { s -= (v - z.mean) / z.sd; n++; } }
            ...
            const pick = (gene, rows) => {
                ...
                const off = gi * nCL;
                return rows.map(r => ({ cl: r.cl, v: this.geneEffects[off + r.i] })).filter(x => !isNaN(x.v));
            };
            const median = (arr) => {
    ...
    _updateGEPlaceholderVisibility() {
        ...
        if (!ph || !tv || !hv) return;
        if (!hasPlot) { ph.style.display = 'flex'; tv.style.display = 'none'; hv.style.display = 'none'; }
    ...
    _checkGEInvariants() {
        ...
        for (const g of annGroups) {
            ...
            if (!titleG && (t.includes('Expression by') || t.includes('Gene Effect by'))) titleG = g;
            if (!yLabelG && /(Mutations|Fusions)\s*$/.test(t) && !t.includes('by')) yLabelG = g;
        }
        const svgRect = svg.getBoundingClientRect();
        let B = titleG ? !(titleG.getBoundingClientRect().left < svgRect.left - 1 || titleG.getBoundingClientRect().right > svgRect.right + 1) : false;
        let C = true;
        if (yLabelG) {
            ...
            for (const t of ticks) {
                ...
                if (!(yr.right < tr.left || tr.right < yr.left || yr.bottom < tr.top || tr.bottom < yr.top)) { C = false; break; }
            }
        }
        return { A, B, C };
    }
    ...
    updateClbFilterCounts(filteredCellLines) {
        ...
        if (this.translocations?.geneData) {
            ...
            if (transDatalist) {
                const geneCounts = [];
                Object.keys(this.translocations.geneData).forEach(gene => {
    ...
    showCellLineDetail(cellLineId) {
        ...
        // Expression section — separator + combined stats + list headings.
        top += `<div style="border-top:1px solid #e5e7eb; margin:12px 0 8px;"></div>`;
        top += `<div class="clb-detail-section"><strong>Expression</strong>`;
        ...
        let gl = `<div class="clb-detail-section"><strong>Lowest Expression (Bottom ${N})</strong> <button class="clb-enrichr-btn" data-list="bottom">Enrichr</button>`;
        ...
        gl += `<div class="clb-detail-section"><strong>Highest Expression (Top ${N})</strong> <button class="clb-enrichr-btn" data-list="top">Enrichr</button>`;
        gl += `<div style="font-size:11px;">`;
        ...
        if (filteredIndices.length >= 3 && clIdx >= 0) {
            ...
            gl += `<div class="clb-detail-section"><strong>Uniquely Low vs ${filterLabel}</strong> <button class="clb-enrichr-btn" data-list="uniqueLow">Enrichr</button>`;
            ...
            gl += `<div class="clb-detail-section"><strong>Uniquely High vs ${filterLabel}</strong> <button class="clb-enrichr-btn" data-list="uniqueHigh">Enrichr</button>`;
    ...
    // Inspect Expression for the currently-selected cell lines. Fast
    // (seconds). Produces two columns: lowest expression in selected, and
    // genes most up vs the rest of the dataset. Clicking a row opens the
    // expression inspect modal for that gene.
    inspectSelectionGE() {
        ...
        document.getElementById('selectionInspectTitle').textContent = `Inspect Expression — ${selected.length} selected cell lines`;
        document.getElementById('selectionInspectSubtitle').textContent = `Left: most highly expressed in selection (ranked by Mean Expr in the selection). Right: largest &Delta; (selection &minus; rest). Columns are sortable; change the |&Delta;| or row-count filters below. Click any gene to open its Expression inspect; use Network to build a correlation network from the displayed genes.`;
        document.getElementById('selectionInspectBody').innerHTML = `
            <div style="display:flex; gap:16px; flex-wrap:wrap; align-items:flex-start;">
                <div style="flex:1; min-width:0;">
                    <div style="display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:6px; flex-wrap:wrap;">
                        <div style="font-weight:600; color:#374151;">Most highly expressed in selection</div>
        ...
        // Independent sort state per side. Default: left sorted by meanSel desc
        // (highest expression first), right sorted by |delta| desc (most different first).
        this._geInspectSort = {
        ...
        const wireRows = () => {
            ...
            document.querySelectorAll('#geLeftBody th.ge-sortable, #geRightBody th.ge-sortable').forEach(th => {
                th.addEventListener('click', () => {
                    ...
                    // default: gene asc, numbers asc, delta absolute desc
                    // (biggest |Δ| first).
                    if (cur.key === key) {
                        cur.dir = -cur.dir;
                    } else {
    ...
    // Build the HTML for one sortable expression-inspect table.
    _buildGEInspectTable(rows, side) {
        ...
        return `<table style="width:100%; border-collapse:collapse; font-size:11px;">
            <thead style="background:#f9fafb; position:sticky; top:0;"><tr>
                ${th('Gene', 'gene', 'left')}
                ${th('Mean Expr (sel)', 'meanSel')}
                ${th('Mean Expr (rest)', 'meanOther')}
                ${th('Δ', 'delta')}
            </tr></thead>
            <tbody>${trows}</tbody>
        </table>`;
    ...
    // requested side of the Inspect Expression modal, restricted to the
    // selected cell lines. Same flow as _launchCorrelationNetwork but
    // starting from a gene list rather than gene pairs.
    ...
    async inspectSelectionCorrelations() {
        ...
        document.getElementById('selectionInspectSubtitle').textContent = `Left: strongest gene-pair expression correlations in the selection. Right: largest Δ (selection r − rest r). Click a row to open the correlation inspect for that pair; use the r-cutoff or the Network button to drill in.`;
        document.getElementById('selectionInspectBody').innerHTML = `
            <div style="display:flex; gap:16px; flex-wrap:wrap; align-items:flex-start;">
                <div style="flex:1; min-width:0;">
                    <div style="display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:6px; flex-wrap:wrap;">
    ...
    exportCellLineBrowserCSV(mode) {
        ...
        if (mode === 'minimal') {
            const headerParts = ['Expression'];
            clNames.forEach(n => headerParts.push(n.replace(/,/g, '')));
            const lines = [`# Expression (TPM log2+1 DepMap) matrix for ${clNames.length} selected cell lines`, `# Source: DepMap 25Q3 OmicsExpressionProteinCodingGenesTPMLogp1`, `# Date: ${new Date().toISOString().slice(0, 10)}`, headerParts.join(',')];
            for (let g = 0; g < this.nGenes; g++) {
            ...
            this.downloadFile(lines.join('\n'), `coexpress${namePart}.csv`, 'text/csv');
        } else {
            const N = parseInt(document.getElementById('clbTopN').value) || 10;
            const filteredCls = this._clbVisibleCellLines;
            const filteredIndices = filteredCls.map(cl => this.metadata.cellLines.indexOf(cl)).filter(i => i >= 0);
            ...
            const lines = ['CellLine,CellLineID,Tissue,Subtype,Category,Rank,Gene,Expression,ZScore'];
            ...
            for (let c = 0; c < clIndices.length; c++) {
                ...
                geneVals.slice(0, N).forEach(({ gene, val }, i) => {
                    lines.push(`${clName},${clId},${tissue},${subtype},Lowest Expression,${i + 1},${gene},${val.toFixed(4)},`);
                });

                geneVals.slice(-N).reverse().forEach(({ gene, val }, i) => {
                    lines.push(`${clName},${clId},${tissue},${subtype},Highest Expression,${i + 1},${gene},${val.toFixed(4)},`);
                });
                ...
                if (filteredIndices.length >= 3) {
                    ...
                    zScores.slice(0, N).forEach(({ gene, val, z }, i) => {
                        lines.push(`${clName},${clId},${tissue},${subtype},Uniquely Low vs ${filterLabel},${i + 1},${gene},${val.toFixed(4)},${z.toFixed(2)}`);
                    });

                    zScores.slice(-N).reverse().forEach(({ gene, val, z }, i) => {
                        lines.push(`${clName},${clId},${tissue},${subtype},Uniquely High vs ${filterLabel},${i + 1},${gene},${val.toFixed(4)},${z.toFixed(2)}`);
                    });
                }
            }

            this.downloadFile(lines.join('\n'), `coexpress_report${namePart}.csv`, 'text/csv');
        }
    }
    ...
    async restoreFromPng(file) {
        ...
        alert('No CoExpress data found in this PNG file.');
    }
    ...
    async restoreFromSvg(file) {
        ...
        alert('No CoExpress data found in this file.');
    }
    ...
    openTextSettings(plotDivId) {
        ...
        body.innerHTML = `
            ...
            <div style="display:flex;flex-wrap:wrap;gap:4px;">
                <button onclick="app._tsColorScheme('essential')" class="ts-color-btn" title="Red/Gray/Green by mean Expression" style="font-size:10px;padding:3px 8px;border:1px solid #d1d5db;border-radius:4px;cursor:pointer;background:linear-gradient(90deg,#dc2626 33%,#9ca3af 33%,#9ca3af 66%,#22c55e 66%);">Essential</button>
                ...
                <button onclick="app._tsColorScheme('blue')" class="ts-color-btn" title="Blue gradient by mean Expression" style="font-size:10px;padding:3px 8px;border:1px solid #d1d5db;border-radius:4px;cursor:pointer;background:linear-gradient(90deg,#1e40af,#93c5fd);">Blue</button>
